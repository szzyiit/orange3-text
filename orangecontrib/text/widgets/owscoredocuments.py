...
class OWScoreDocuments(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        ...
        words = Input("词(Words)", Table, replaces=['Words'])

    class Outputs:
        selected_documents = Output("选中的文档(Selected Documents)", Corpus, replaces=['Selected Documents'], default=True)
    ...
    def _setup_control_area(self) -> None:
        box = gui.widgetBox(self.controlArea, "词评分方法")
        ...
        box = gui.widgetBox(self.controlArea, "聚合")
        gui.comboBox(
            box,
            self,
            "aggregation",
        ...
        # select words box
        box = gui.vBox(self.buttonsArea, "选择文档")
        grid = QGridLayout()
    ...
    def _setup_main_area(self) -> None:
        self._filter_line_edit = QLineEdit(
            textChanged=self.__on_filter_changed, placeholderText="筛选..."
        )
