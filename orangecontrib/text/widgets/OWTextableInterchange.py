...
class OWTextableInterchange(OWTextableBaseWidget):
    ...
    name = "转换(Interchange)"
    description = "XML 提取的数据与语料数据格式转换"
    icon = "icons/interchange.svg"
    priority = 10003

    class Inputs:
        segmentation = Input("分段(Segmentation)", Segmentation, replaces=["Segmentation"])
        corpus = Input("语料(Corpus)", Corpus, replaces=["Corpus"])

    class Outputs:
        segmentation = Output('分段(Segmentation)', Segmentation, replaces=['Segmentation'])
    ...
    def sendData(self):
        ...
        if self.segmentation:
            ...
            metas.append(StringVariable(f"text"))
            domain = Domain(attributes, [], metas)
            rows = list()
            for segment in self.segmentation:
                row = [
                    str(segment.annotations.get(annotation_key, ''))
