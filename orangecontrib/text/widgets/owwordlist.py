...
class OWWordList(OWWidget):
    ...
    class Outputs:
        selected_words = Output("选中的词(Selected Words)", Table, replaces=['Selected Words'])
        words = Output("词(Words)", Table, replaces=['Words'])
    ...
    def __init__(self):
        ...
        self.__library_box: QGroupBox = gui.vBox(None, "库")
        self.__input_box: QGroupBox = gui.vBox(None, "输入")
    ...
    def _setup_library_box(self):
        ...
        new_from_file = QAction("从文件导入词", self)
        new_from_file.triggered.connect(self.__on_import_word_list)

        save_to_file = QAction("将词保存为文件", self)
