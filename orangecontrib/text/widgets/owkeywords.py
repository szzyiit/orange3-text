...
class OWKeywords(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        corpus = Input("语料(Corpus)", Corpus, replaces=['Corpus'], default=True)
        words = Input("词(Words)", Table, replaces=['Words'])

    class Outputs:
        words = Output("词(Words)", Corpus, replaces=['Words'])
    ...
    def _setup_gui(self):
        grid = QGridLayout()
        box = gui.widgetBox(self.controlArea, "评分方法", grid)
        ...
        box = gui.vBox(self.buttonsArea, "选词")
        grid = QGridLayout()
