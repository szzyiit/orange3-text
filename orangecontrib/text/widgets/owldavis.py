...
class BarPlotGraph(pg.PlotWidget):
    ...
    def update_graph(
        ...
        self.marg_prob_item = pg.BarGraphItem(
            ...
            brushes=[
                self.colors["总体词频"] for _ in marginal_probability
            ],
            pen=self.colors["总体词频"],
        )
        self.term_topic_freq_item = pg.BarGraphItem(
            ...
            brushes=[
                self.colors["此主题词频"] for _ in term_topic_freq
            ],
            pen=self.colors["此主题词频"],
        )
    ...
    def update_axes(self, words: List[str]) -> None:
        ...
        self.setLabel(axis="left", text="词")
        self.setLabel(axis="top", text="权重")
...
class OWLDAvis(OWWidget):
    ...
    class Inputs:
        topics = Input("主题(Topics)", Topics, replaces=['Topics'])
    ...
    def _create_layout(self):
        self._add_graph()
        box = gui.widgetBox(self.controlArea, "相关")
